...
def int_scenario_log_probability(index, success_probs):
    ...
    return np.sum(np.log(scenario * success_probs + (1 - scenario) * (1 - success_probs)))


def flip_scenario(int_scenario, scenario_prob, index, success_prob, scenario_length):
    ...
    Returns:
        a pair `(log_prob, scenario)` with `scenario` being the new
        scenario in bitstring form and `log_prob` being the associated
...
def possible_next_scenarios(bitstr, scenario_prob, success_probs):
    ...
    for flip_pos in range(scenario_length):
        new_prob, new_scenario = flip_scenario(bitstr, scenario_prob, flip_pos,
                                               success_probs[flip_pos],
                                               success_probs.size)
...
def get_top_scenarios(n, success_probs):
    ...
    q = [(-int_scenario_log_probability(top_scenario, success_probs), top_scenario)]
